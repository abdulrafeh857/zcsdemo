...
public class SLE4428Activity extends BaseActivity implements View.OnClickListener {
    ...
    private SLE4428Card mSLE4428Card = DriverManager.getInstance().getCardReadManager().getSLE4428Card();
    ...
    private void writeCombi() {
        ...
        DialogUtils.showViewDialog(this, view, getString(R.string.input_byte), null,
                getString(android.R.string.ok), getString(android.R.string.cancel), new DialogInterface.OnClickListener() {
                    @Override
    ...
    private void writeProtected() {
        ...
        int ret = mSLE4428Card.writeProtectedData((byte) 0, (byte) 32, protectedData);
    ...
    private void readErrCount() {
        byte[] key = new byte[3];
        int ret = mSLE4428Card.readErrCountAndKey(key);
        Log.e(TAG, "readErrCountAndKey: " + ret + "\t" + StringUtils.convertBytesToHex(key));
        if (ret == SdkResult.SDK_OK) {
            int count = mSLE4428Card.parseErrorCount(key[0]);
    ...
    private void initView() {
        ...
        if (actionBar != null) {
            actionBar.setTitle(R.string.pref_sle4428);
            actionBar.setDisplayHomeAsUpEnabled(true);
        }
        mTvLog = (TextView) findViewById(R.id.tv_log);
        mScrollView = (ScrollView) findViewById(R.id.scrollView);
        mBtnRead = (Button) findViewById(R.id.btn_read);
        mBtnRead.setOnClickListener(SLE4428Activity.this);
        mBtnWrite = (Button) findViewById(R.id.btn_write);
        mBtnWrite.setOnClickListener(SLE4428Activity.this);
        mBtnErrorCount = (Button) findViewById(R.id.btn_error_count);
        mBtnErrorCount.setOnClickListener(SLE4428Activity.this);
        mBtnChangeKey = (Button) findViewById(R.id.btn_change_key);
        mBtnChangeKey.setOnClickListener(SLE4428Activity.this);
        mBtnVerify = (Button) findViewById(R.id.btn_verify);
        mBtnVerify.setOnClickListener(SLE4428Activity.this);
        mBtnInit = (Button) findViewById(R.id.btn_init);
        mBtnInit.setOnClickListener(SLE4428Activity.this);
